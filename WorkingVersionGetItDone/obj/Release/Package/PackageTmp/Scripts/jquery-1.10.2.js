...
jQuery.extend({
	...
	nodeName: function( elem, name ) {
		return elem.nodeName && elem.nodeName === name;
	},
	...
	filter: {
		...
		"TAG": function( nodeNameSelector ) {
			...
			return nodeNameSelector === "*" ?
				...
				function( elem ) {
					return elem.nodeName && elem.nodeName === nodeName;
				};
		},
					...
					if ( parent ) {
						...
						if ( simple ) {
							while ( dir ) {
								node = elem;
								while ( (node = node[ dir ]) ) {
									if ( ofType ? node.nodeName === name : node.nodeType === 1 ) {
										return false;
									}
								}
						...
						// xml :nth-child(...) or :nth-last-child(...) or :nth(-last)?-of-type(...)
							...
							while ( (node = ++nodeIndex && node && node[ dir ] ||
								...
								if ( ( ofType ? node.nodeName === name : node.nodeType === 1 ) && ++diff ) {
		...
		"PSEUDO": function( pseudo, argument ) {
			...
			var args,
				fn = Expr.pseudos[ pseudo ] || Expr.setFilters[ pseudo ] ||
	...
	pseudos: {
		...
		"text": function( elem ) {
			...
			return elem.nodeName === "input" &&
				elem.type === "text" &&
				( (attr = elem.getAttribute("type")) == null || attr === elem.type );
		},
...
if ( !assert(function( div ) {
	...
	addHandle( "type|href|height|width", function( elem, name, isXML ) {
		if ( !isXML ) {
			return elem.getAttribute( name, name === "type" ? 1 : 2 );
		}
	});
}
...
jQuery.extend({
	...
	acceptData: function( elem ) {
		...
		var noData = elem.nodeName && jQuery.noData[ elem.nodeName ];
...
function addToPrefiltersOrTransports( structure ) {
	...
	return function( dataTypeExpression, func ) {
		...
		var dataType,
			i = 0,
			dataTypes = dataTypeExpression.match( core_rnotwhite ) || [];
...
jQuery.extend({
	...
	ajax: function( url, options ) {
		...
		s.dataTypes = jQuery.trim( s.dataType || "*" ).match( core_rnotwhite ) || [""];
...
function ajaxConvert( s, response, jqXHR, isSuccess ) {
	...
	if ( dataTypes[ 1 ] ) {
		for ( conv in s.converters ) {
			converters[ conv ] = s.converters[ conv ];
		}
	}
